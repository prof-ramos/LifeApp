...
const API_URL = (runtimeEnv?.EXPO_PUBLIC_API_URL || 'http://127.0.0.1:4173').replace(/\/$/, '');
...
const token = () => SecureStore.getItemAsync(SESSION_KEY);
const saveToken = (value: string) => SecureStore.setItemAsync(SESSION_KEY, value, {keychainAccessible: SecureStore.AFTER_FIRST_UNLOCK_THIS_DEVICE_ONLY});
const clearToken = () => SecureStore.deleteItemAsync(SESSION_KEY);
...
export async function bootstrapSession(): Promise<SessionResponse> {
  ...
  try {
    return await fetchJson<SessionResponse>('/api/mobile/session', {}, saved);
  } catch (error) {
