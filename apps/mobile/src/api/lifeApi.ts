...
export type ServerSession={role:'resident';cashbackCents:number;orders:number;condoRevenueCents:number;legalAcceptedVersion:string|null};
...
async function saveToken(value:string){await SecureStore.setItemAsync(SESSION_KEY,value,{keychainAccessible:SecureStore.AFTER_FIRST_UNLOCK_THIS_DEVICE_ONLY})}
...
async function fetchJson<T>(path:string,init:RequestInit={},sessionToken?:string|null):Promise<T>{
  ...
  if(!response.ok){const error=new Error(payload?.error?.code||'REQUEST_FAILED') as Error&{status?:number};error.status=response.status;throw error}
  return payload as T;
}
...
export async function acceptLegalVersion(version:string):Promise<ServerSession>{
  const saved=await token();if(!saved)throw new Error('UNAUTHENTICATED');
...
export async function checkoutProduct(productId:string,useCashback:boolean,idempotencyKey:string):Promise<{session:ServerSession;allocation:{customerPayableCents:number;cashbackUsedCents:number;cashbackEarnedCents:number;condominiumShareCents:number}}>{
  const saved=await token();if(!saved)throw new Error('UNAUTHENTICATED');
  return fetchJson('/api/checkout/commit',{method:'POST',headers:{'idempotency-key':idempotencyKey},body:JSON.stringify({productId,useCashback})},saved);
}
