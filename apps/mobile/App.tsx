...
export default function App(){
  ...
  useEffect(()=>{
    let active=true;
    loadLocalLifeState().then(state=>{if(!active)return;localRef.current=state;setCashback(state.cashback);setOrders(state.orders);setLegalAcceptedVersion(state.legalAcceptedVersion)}).catch(()=>{}).finally(()=>{if(active)setHydrated(true)});
    return()=>{active=false};
  },[]);

  const acceptLegal=async()=>{const next={...localRef.current,legalAcceptedVersion:LEGAL_VERSION};localRef.current=next;setLegalAcceptedVersion(LEGAL_VERSION);await saveLocalLifeState(next)};
  const buy=async(m:Merchant)=>{if(purchaseLock.current)return;purchaseLock.current=true;setIsPurchasing(true);try{const current=localRef.current;const cashbackUsed=useCashback?money(Math.min(current.cashback,m.price*.5)):0;const customerPayable=money(m.price-cashbackUsed);const earned=money(customerPayable*(m.cashback/100));const next={cashback:money(current.cashback-cashbackUsed+earned),orders:current.orders+1,legalAcceptedVersion:current.legalAcceptedVersion};localRef.current=next;setCashback(next.cashback);setOrders(next.orders);setUseCashback(false);await saveLocalLifeState(next);setMessage(`Pagamento MVP aprovado: ${brl(customerPayable)} · cashback usado ${brl(cashbackUsed)} · ganho ${brl(earned)}.`)}finally{purchaseLock.current=false;setIsPurchasing(false)}};

  const content=useMemo(()=>{
    if(tab==='market')return <><SectionTitle eyebrow="Perto de você" title="Marketplace" sub="Produtos e serviços próximos. Pagamento sempre dentro do Life."/><Card className="mb-4 bg-card-elevated"><View className="flex-row items-center justify-between"><View><CardTitle>Life Wallet</CardTitle><Text className="mt-1 text-3xl font-black text-foreground">{brl(cashback)}</Text></View><Button variant={useCashback?'outline':'default'} onPress={()=>setUseCashback(v=>!v)}>{useCashback?'Remover':'Aplicar'}</Button></View><CardDescription className="mt-3">No MVP, até 50% de cada compra pode ser coberto com cashback.</CardDescription></Card>{merchants.map(m=><MerchantCard key={m.id} {...m} redeemed={useCashback?money(Math.min(cashback,m.price*.5)):0} processing={isPurchasing} onBuy={()=>void buy(m)}/>)}</>;
