...
export default function App(){
  ...
  const buy=async(m:Merchant)=>{
    ...
    const cashbackUsed=useCashback?money(Math.min(current.cashback,m.price*.5)):0;
    const customerPayable=money(m.price-cashbackUsed);
    const earned=money(customerPayable*(m.cashback/100));
    const next:LocalLifeState={cashback:money(current.cashback-cashbackUsed+earned),orders:current.orders+1,legalConsent:current.legalConsent};
    try{
      await persistNext(next);
      setUseCashback(false);
      setMessage(`Pagamento MVP aprovado: ${brl(customerPayable)} · cashback usado ${brl(cashbackUsed)} · ganho ${brl(earned)}.`);
    }catch{
  ...
  },[tab,cashback,orders,legalConsent,useCashback,isPurchasing]);
...
function CheckRow({checked,label,onPress}:{checked:boolean;label:string;onPress:()=>void}){return <Pressable accessibilityRole="checkbox" accessibilityState={{checked}} onPress={onPress} className="mt-4 flex-row items-center"><Text className="mr-3 text-2xl text-primary">{checked?'☑':'☐'}</Text><Text className="flex-1 text-sm leading-5 text-foreground">{label} · {LEGAL_VERSION}</Text></Pressable>}
