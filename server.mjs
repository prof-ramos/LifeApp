...
function newSession(){const token=randomUUID();sessions.set(token,{token,role:'resident',cashbackCents:2780,orders:0,condoRevenueCents:0,legalAcceptedVersion:null,createdAt:Date.now(),idempotency:new Map()});return sessions.get(token)}
function getSession(req,{create=false}={}){const token=bearer(req)||parseCookies(req).life_session;let session=token?sessions.get(token):null;if(session&&Date.now()-session.createdAt>SESSION_TTL_MS){sessions.delete(token);session=null}return session||create?session||newSession():null}
...
function checkRate(req,key,limit,windowMs){const bucketKey=`${clientIp(req)}:${key}`;const now=Date.now();const prev=rateBuckets.get(bucketKey);if(!prev||now-prev.start>=windowMs){rateBuckets.set(bucketKey,{start:now,count:1});return true}if(prev.count>=limit)return false;prev.count+=1;return true}
...
createServer(async(req,res)=>{
  try{
    ...
    if(pathname==='/api/checkout/quote'&&req.method==='POST'){
      ...
      if(session.idempotency.has(idempotencyKey))return json(res,200,session.idempotency.get(idempotencyKey));
      const input=await readJson(req);
      ...
      session.idempotency.set(idempotencyKey,payload);
...
}).listen(4173,'127.0.0.1',()=>console.log('Life MVP demo: http://127.0.0.1:4173'));
