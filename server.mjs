...
function requireRole(session,res,role){if(session.role!==role){apiError(res,403,'FORBIDDEN','Operação não permitida para este perfil.');return false}return true}
function requireSameOriginForCookie(req,res){if(bearer(req))return true;const origin=req.headers.origin;if(!origin){apiError(res,403,'ORIGIN_REQUIRED');return false}try{const parsed=new URL(origin);const host=String(req.headers.host||'');if(parsed.host!==host){apiError(res,403,'ORIGIN_MISMATCH');return false}}catch{apiError(res,403,'INVALID_ORIGIN');return false}return true}
...
const server=createServer(async(req,res)=>{
  try{
    ...
    // Browser session: HttpOnly cookie only. Never exposes the cookie session token to JS.
    if(pathname==='/api/session'&&req.method==='GET'){
      if(bearer(req))return apiError(res,400,'USE_MOBILE_SESSION_ENDPOINT');
      const session=getSession(req,{createCookieSession:true});
      return json(res,200,{session:publicSession(session),legalVersion:LEGAL_VERSION},{'set-cookie':setSessionCookie(session)});
    }
    ...
    if(pathname==='/api/checkout/commit'&&req.method==='POST'){
      ...
      const session=requireSession(req,res);if(!session||!requireLegal(session,res)||!requireRole(session,res,'resident'))return;
...
server.listen(PORT,'127.0.0.1',()=>console.log(`Life MVP demo: http://127.0.0.1:${PORT}`));
