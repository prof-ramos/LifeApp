...
function market(){return `<h1>Marketplace</h1><p>Pagamento exclusivamente dentro do Life.</p><div class="grid">${merchants.map(m=>`<section class="card"><span class="badge">${m.cashback}% cashback</span><h3>${m.name}</h3><p>${m.category} · ★ ${m.rating}</p><strong>${m.item} · ${brl(m.price)}</strong><button onclick="buy(${m.id})">Comprar no Life</button></section>`).join('')}</div>`}
